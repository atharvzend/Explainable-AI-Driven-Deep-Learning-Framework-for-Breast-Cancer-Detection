"""
Evaluation Metrics — BreakHis Multiclass (8 tumor subtypes)
============================================================
Usage:
  python evaluate/metrics_multiclass.py --model cnn_vit
  python evaluate/metrics_multiclass.py --model efficientnet_vit
"""
...
def compute_metrics(preds, labels, probs):
    ...
    try:
        ...
        auc_per_class_vals = roc_auc_score(labels, probs, multi_class="ovr", average=None)
        auc_per_class = {CLASS_NAMES[i]: float(auc_per_class_vals[i]) for i in range(NUM_CLASSES)}
    except Exception:
        auc = float("nan")
        auc_per_class = {c: float("nan") for c in CLASS_NAMES}
...
def evaluate_model(model, test_loader, per_mag_loaders, model_name, device):
    ...
    plot_confusion_matrix(np.array(overall["confusion_matrix"]), model_name, "Overall Combined")
    print_metrics(overall, f"{model_name} | Overall (40X+100X+200X+400X) | 8-class")
    full_results["overall"] = overall

    per_mag = {}
    from data.breakhis_multiclass_dataset import MAGNIFICATIONS
    for mag in MAGNIFICATIONS:
        if mag not in per_mag_loaders:
            continue
        preds_m, labels_m, probs_m = collect_predictions(model, per_mag_loaders[mag], device)
        mag_metrics = compute_metrics(preds_m, labels_m, probs_m)
        plot_confusion_matrix(np.array(mag_metrics["confusion_matrix"]), model_name, f"BreakHis {mag}")
        print_metrics(mag_metrics, f"{model_name} | {mag} | 8-class")
...
if __name__ == "__main__":
    ...
    _, _, test_loader, per_mag_loaders, _ = get_multiclass_dataloaders()
    ...
    model = CNNViT(num_classes=NUM_CLASSES) if args.model == "cnn_vit" \
            else EfficientNetViT(num_classes=NUM_CLASSES)
